...
function toLoginResult(user, additionalUserInfo?): User {
  ...
  for (let i = 0; i < providerData.size(); i++) {
    ...
    if (pid === 'facebook.com') {
      ...
      providers.push({ id: pid, token: firebase._twitterSignInIdToken });
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (arg.type === firebase.LoginType.ANONYMOUS) {
        ...
        const onSuccessListener = new gmsTasks.OnSuccessListener({
          onSuccess: (authResult: com.google.firebase.auth.AuthResult) => {
            firebase._twitterSignInIdToken = (<any>authResult.getCredential()).getIdToken();
            const loginResult = toLoginResult(authResult.getUser(), authResult.getAdditionalUserInfo());
            firebase.notifyAuthStateListeners({
              loggedIn: true,
              user: loginResult
            ...
            resolve(loginResult);
            // TODO for reauth and linking, see https://firebase.google.com/docs/auth/android/twitter-login
          }
        });
        ...
        if (pendingAuthResult) {
          ...
          // no pending result; start the sign in flow
          const oAuthProviderBuilder = com.google.firebase.auth.OAuthProvider.newBuilder("twitter.com");

          let scopes = [];
          if (arg.twitterOptions && arg.twitterOptions.scopes) {
            scopes = arg.twitterOptions.scopes;
          }
          oAuthProviderBuilder.setScopes(firebase.toJavaArray(scopes));

          if (arg.twitterOptions && arg.twitterOptions.lang) {
            oAuthProviderBuilder.addCustomParameter("lang", arg.twitterOptions.lang);
          }
